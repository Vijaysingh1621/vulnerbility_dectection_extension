...
export function detectEmergingThreats(code: string) {
    const issues = [];
  
    // SSRF Detection
    if (/fetch\(['"`]http/.test(code)) {
      issues.push({
        line: findLineNumber(code, 'fetch'),
    ...
    if (/['"`](apiKey|password)['"`]:/.test(code)) {
      issues.push({
        line: findLineNumber(code, 'apiKey'),
  ...
  if (/predict\(.+\)/.test(code) && !/validateInput/.test(code)) {
      issues.push({
          line: findLineNumber(code, 'predict'),
  ...
  if (/aws\.S3/.test(code) && !/acl: "private"/.test(code)) {
      issues.push({
          line: findLineNumber(code, 'aws.S3'),
  ...
  if (/req\.headers\['authorization'\]/.test(code) && !/checkPermissions/.test(code)) {
      issues.push({
          line: findLineNumber(code, 'authorization'),
  ...
  if (/docker\.run/.test(code) && /:latest/.test(code)) {
      issues.push({
          line: findLineNumber(code, 'docker.run'),
  ...
  if (/connectDevice\(.+\)/.test(code) && !/encrypt/.test(code)) {
      issues.push({
          line: findLineNumber(code, 'connectDevice'),
  ...
  if (/web3\.eth/.test(code) && !/validateTransaction/.test(code)) {
      issues.push({
          line: findLineNumber(code, 'web3.eth'),
