...
export function detectSANS(code: string) {
    const issues = [];
...
if (/strcpy\(.+\)/.test(code)) {
  issues.push({
      line: findLineNumber(code, 'strcpy'),
...
if (/const\s+\w+\s*=\s*['"].+['"];/.test(code) && /password|username|key/i.test(code)) {
  issues.push({
      line: findLineNumber(code, 'const'),
...
if (/fs\.readFileSync\s*\(.+userInput.+\)/.test(code)) {
  issues.push({
      line: findLineNumber(code, 'fs.readFileSync'),
...
if (/fs\.writeFile\s*\(.+\)/.test(code) && !/try/.test(code)) {
  issues.push({
      line: findLineNumber(code, 'fs.writeFile'),
...
if (/fs\.chmod\s*\(.+['"]777['"]\)/.test(code)) {
  issues.push({
      line: findLineNumber(code, 'fs.chmod'),
    ...
    if (/new\s+Buffer\s*\(/.test(code)) {
      issues.push({
          line: findLineNumber(code, 'new Buffer'),
  ...
  if (/Math\.pow\(.+\)/.test(code) && /int\s*/.test(code)) {
      issues.push({
          line: findLineNumber(code, 'Math.pow'),
  ...
  if (/while\s*\(.+\)/.test(code) && !/break;/.test(code)) {
      issues.push({
          line: findLineNumber(code, 'while'),
  ...
  if (/https\.request\(.+\)/.test(code) && code.includes('rejectUnauthorized: false')) {
      issues.push({
          line: findLineNumber(code, 'https.request'),
  ...
  if (/process\.setuid\(.+\)/.test(code)) {
      issues.push({
          line: findLineNumber(code, 'process.setuid'),
  ...
  if (/fs\.open\(.+\)/.test(code) && !code.includes('.close')) {
      issues.push({
          line: findLineNumber(code, 'fs.open'),
  ...
  if (/file\.mimetype/.test(code) && !/\.jpg|\.png|\.gif|\.txt/.test(code)) {
      issues.push({
          line: findLineNumber(code, 'file.mimetype'),
  ...
  if (/response\.send\(.+userInput.+\)/.test(code)) {
      issues.push({
          line: findLineNumber(code, 'response.send'),
